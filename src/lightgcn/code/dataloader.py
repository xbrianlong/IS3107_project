...
class MusicDataset(BasicDataset):
    def __init__(self, path=os.path.abspath("src/lightgcn/data/music")): # might have to double check dir paths
        ...
        self.n_user = self.trainUser.max() + 1
        self.m_item = self.trainItem.max() + 1

        self.testUser = np.array(test_users)
        self.testItem = np.array(test_items)
    ...
    def _convert_sp_mat_to_sp_tensor(self, X):
        ...
        return torch.sparse.FloatTensor(index, data, torch.Size(coo.shape)).coalesce().to(world.device)

    def __build_test(self):
        test_data = {}
        for u, i in zip(self.testUser, self.testItem):
            test_data.setdefault(u, []).append(i)
        return test_data

    def __getitem__(self, index):
        return self.trainUser[index]

    def __len__(self):
        return len(np.unique(self.trainUser))
